...
test('api', (t) => {
	...
	const allAreFunctions = api.reduce((allAreFunctions_, key) => {
		// eslint-disable-next-line security/detect-object-injection
		const isFunction = typeof mod[key] === 'function';
		if (!isFunction) {
			t.log({ key, isFunction });
		}
		return allAreFunctions_ && isFunction;
	}, true);
	t.true(allAreFunctions);
});
